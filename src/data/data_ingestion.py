...
class DataIngestion:
    ...
    def load_table(self, table_name: str) -> pd.DataFrame:
        """load a complete table into a pandas dataframe"""
        try:
            query = text(f"SELECT * FROM {table_name}")

            dataframe = pd.read_sql(query, self.engine)

            logging.info(f"loaded {table_name} table, it contains {dataframe.shape[0]} rows and {dataframe.shape[1]} columns")
    ...
    def load_all_data(self):
        try:
            ...
            logging.error(f"error occurred while loadding all dataset {e}")
            raise MyException(e, sys)
    ...
    def data_merger(self):
        ...
        try:
            ...
            logging.info(f"the dataset has been succesfully merged {customer_review_data.shape}")
            return customer_review_data
        except Exception as e:
            logging.error("error occurred while merging the data {e}")
            raise MyException(e, sys)
