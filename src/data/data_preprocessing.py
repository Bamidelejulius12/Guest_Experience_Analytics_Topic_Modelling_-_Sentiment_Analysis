...
class SentimentAnalysisProcessor:
    ...
    def split_data(self, dataframe):
        ...
        try:
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                ...
                test_size=0.2,
                random_state=42,
                stratify=y_encoded   # keeps class balance consistent across splits
            )
            ...
            logging.error(f"error occurred while splitting the data into training and testing {e}")
    ...
    def tokenize_data(self, X_train, X_test):
        ...
        try:
            ...
            logging.info(f"successfully tokenized our training and testing dataset")
            return train_encodings, test_encodings
        except Exception as e:
            logging.error(f"error occurred while tokenizing the dataset {e}")
            raise MyException(e, sys)
    ...
    def create_dataset(self,  train_encodings, test_encodings, y_train, y_test):
        try:
            ...
            logging.error(f"error occureed while created the dataset {e}")
            raise MyException(e, sys)
