...
from config.constant import model_name, classification_model_name, experiment_name, training_args
...
class ModelTracker:
    def __init__(self, experiment_name = experiment_name):
        try:
            ...
            logging.error(f"error occurred while initalizing mlflow {e}")
    ...
    def model_update_tracker(self, trainer, metrics):
        try:
            ...
            if old_f1 is None or new_f1 > old_f1:
                ...
                logging.info("new model and tokenizer have been registered in MLFLOW")

            else:
                logging.info("Model not  pushed (performance didn't improve)")

        except Exception as e:
            logging.error(f"error occurred while pushing the model to mlflow {e}")
            raise MyException(e, sys)
